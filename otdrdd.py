...
m_col1.metric(label="Потери в волокне", value=f"{fiber_loss:.2f} дБ")
m_col2.metric(label="Расчетный запас (Margin)", value=f"{margin:.2f} дБ")
m_col3.metric(label="Живой ДД трассы", value=f"{required_trace_dr:.2f} дБ")

# Главный итог
st.info(f"**Влияние настроек прибора на энергетику:**\n"
        f"* Изменение ДД из-за длительности импульса ($D_i$): `{D_i:.2f} дБ`\n"
        f"* Изменение ДД из-за времени накопления ($D_t$): `{D_t:.2f} дБ`")

# Выделяем самый важный результат большим баннером
st.success(f"### ТРЕБУЕМЫЙ ПАСПОРТНЫЙ ДИАПАЗОН ПРИБОРА: **{required_passport_dr:.2f} дБ**")
