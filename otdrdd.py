...
# Общая формула с подставленными значениями (неяркая)
st.write(
    f":grey[Расчетная формула: Потери + Запас - D_i - D_t = "
    f"{fiber_loss:.2f} + {margin:.2f} - ({D_i:.2f}) - ({D_t:.2f})]"
)

# Финальная строка: обычный размер шрифта, чтобы избежать переноса "дБ", 
# но с ярким цветовым и жирным выделением самого результата расчета
st.write(f"ТРЕБУЕМЫЙ ПАСПОРТНЫЙ ДИАПАЗОН ПРИБОРА: :red[**{required_passport_dr:.2f} дБ**]")

# Обновленное примечание с предупреждением о соединениях
st.caption(
    "Примечание: данный диапазон должен быть указан в технических характеристиках (паспорте) "
    "рефлектометра для стандартных условий измерений (20 мкс / 3 мин). В данном расчете не "
    "учитываются дополнительные потери, вносимые разъемными и неразъемными соединениями, "
    "вероятно присутствующими на линии."
)