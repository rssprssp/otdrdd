...
# Промежуточные строки выводятся неярким серым цветом
st.write(f":grey[Потери в волокне: {fiber_loss:.2f} дБ]")
st.write(f":grey[Расчетный запас (Margin): {margin:.2f} дБ]")
st.write(f":grey[Живой ДД трассы: {required_trace_dr:.2f} дБ]")
st.write(f":grey[Изменение ДД из-за длительности импульса (D_i): {D_i:.2f} дБ]")
st.write(f":grey[Изменение ДД из-за времени накопления (D_t): {D_t:.2f} дБ]")

# Последняя строка визуально выделена стандартным цветом, крупно и жирно
st.subheader(f"ТРЕБУЕМЫЙ ПАСПОРТНЫЙ ДИАПАЗОН ПРИБОРА: {required_passport_dr:.2f} дБ")
